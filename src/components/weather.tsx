...
export const Weather = () => {
  ...
  const [weatherForecast1, setWeatherForecast1] = useState<WeatherDataInterface[]>([]);
  const [weatherForecast2, setWeatherForecast2] = useState<WeatherDataInterface[]>([]);
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        ...
        const fetchWeatherForecast = async ( cities: string[] ) => {
            const datas1 = await getForecastData( cities, 1 );
            setWeatherForecast1(datas1);
            const datas2 = await getForecastData( cities, 2 );
            setWeatherForecast2(datas2);
        }
  ...
  return (
    <div>
        ...
        <Modal isOpen={isOpen} onRequestClose={()=>setIsOpen(false)}>
            ...
            <div>
                <h2 className="text-2xl font-bold text-center mb-4">1 day weather forecast</h2>
                <div className="flex flex-wrap justify-center items-start gap-4">
                    {
                        weatherForecast1.map( (item, index) => <DisplayWeather key={index} city={item} /> )
                    }
                </div>
            </div>
            <div>
                <h2 className="text-2xl font-bold text-center mb-4">2 days weather forecast</h2>
                <div className="flex flex-wrap justify-center items-start gap-4">
                    {
                        weatherForecast2.map( (item, index) => <DisplayWeather key={index} city={item} /> )
                    }
                </div>
            </div>
